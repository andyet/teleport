...
| `--insecure-no-tls`| none | none | disable TLS for the [web proxy](./architecture/#the-proxy-service)
...
| `--nodename`    | `hostname` | **string** | name this node <!--TODO link describe function of nodename-->
...
**Caution**: This sample config is _not_ the default config and should be used for reference only. <!--TODO Link to Config Doc-->
...
**Usage** `usage: tsh ls [<flags>] [<label>]` <!--TODO: label? or labels? seems like it only supports one label at a time-->
...
| `--token` | none | **string** token value | A custom token to use, autogenerated if not provided. Should match token set with `teleport start --token`
...
`--compat` | none | version number | export cerfiticates compatible with specific version of Teleport
...
| Name | Default Value(s) | Allowed Value(s) | Description
|------|---------|----------------|----------------------------|
`--grace-period` | none | relative duration like 5s, 2m, or 3h |  Grace period keeps previous certificate authorities signatures valid, if set to 0 will force users to relogin and nodes to re-register.
